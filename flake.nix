{
  ...
  outputs = inputs@{ self, nixpkgs, flake-utils, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        packages = flake-utils.lib.flattenTree (perGHC // {
          inherit binaries;
        ...
        devShells =
          let
            shellFor = packages: perGHC.${defaultGHCVersion}.dev.hsPkgs.shellFor {
              inherit packages;
              tools = {
                cabal = "latest";
              ...
              withHoogle = false;
              exactDeps = false;
              inherit (self.packages.${system}.pre-commit-check) shellHook;
            };
          in
          {
            default = shellFor (ps: [ ps.ormolu ]);
            extractHackageInfo = shellFor (ps: [ ps.extract-hackage-info ]);
          };
        legacyPackages = perGHC.${defaultGHCVersion};
      });
  nixConfig = {
    extra-substituters = [
      "https://cache.iog.io"
